...
public class MapServlet extends HttpServlet {
	...
	public void getAddressInfo(HttpServletRequest request, HttpServletResponse response,String mode) throws ServletException, IOException
	{
		...
		else if(mode.equals("tupyogu") || mode.equals("tong"))
		{
			...
			{
				sql = "SELECT DISTINCT '��' || TO_NUMBER(REPLACE(REPLACE(tupogu, '��', ''),'��ǥ��', '')) || '��ǥ��' AS tupogu FROM DATAADDRESS	where haengjoungdong =?"
						+ "ORDER BY TO_NUMBER(REPLACE(REPLACE(tupogu, '��', ''),'��ǥ��', '')) asc";
			}
			else
			{
			...
			*/
			...
			// Mysql
			if(mode.equals("tupyogu")) {
				sql = "SELECT DISTINCT CONCAT(CONCAT('��',REPLACE(REPLACE(TUPYOGU, '��', ''),'��ǥ��', '')),'��ǥ��') AS TUPYOGU from DATAADDRESS where HAENGJOUNGDONG = ?"
						+ "ORDER BY CAST(REPLACE(REPLACE(TUPYOGU, '��', ''),'��ǥ��', '') AS UNSIGNED) asc";
			}
			else {
